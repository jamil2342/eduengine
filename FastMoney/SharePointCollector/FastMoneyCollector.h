...
#include "../SharePointAdapter/SharePointAdapter.h"
...
class FastMoneyCollector
{
	int m_iDataCounter;
public:
	//__declspec(dllexport) void RetrieveListItems1(wchar_t* url, VARIANT *vValues, int &ret)
	//{
	//	ISharePointAdapter * share;// = new ISharePointAdapter();

	//	SharePointClient ^ _managedObject = gcnew SharePointClient();
	...
	__declspec(dllexport)  int GetFastMoneyDataTable(wchar_t* Url)
	{
		...
		if (true)
		{
			...
			//CSharePointAdapter ^ share = gcnew CSharePointAdapter();
			CSharePointAdapter * share = new CSharePointAdapter();
			share->RetrieveListItems1(Url, &vtItems, nres);
			//m_pOwner->m_pSharePoint->RetrieveListItems(_bstr_t(m_pOwner->m_base.m_szListTitle), &vtItems, nres);
			//RetrieveListItems1(Url, &vtItems, nres);
			//#####do correct
